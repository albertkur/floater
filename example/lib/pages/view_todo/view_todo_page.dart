...
class ViewTodoPage extends StatefulWidgetBase<ViewTodoPageState> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(centerTitle: true, title: Text("View Todo"), backgroundColor: this.state.appBarColor),
  ...
  Widget _buildBody(BuildContext context) {
    ...
    return Center(
      child: Column(
        ...
        children: [
          Text(this.state.todo.title, style: Theme.of(context).textTheme.headlineSmall),
          if (this.state.todo.description != null)
            Text(this.state.todo.description!, style: Theme.of(context).textTheme.bodyMedium),
        ],
      ),
    );
  }
